(() => {
  ...
  async function loadAccounting(){
    ...
    $('#categoryTable').innerHTML=table(['Category','Type','Status','Actions'],(state.accounting.allCategories||[]).map(c=>`<tr class="${c.active?'':'rule-disabled'}"><td><strong>${esc(c.name)}</strong></td><td>${esc(c.behavior)}</td><td>${c.active?'Enabled':'Disabled'}</td><td><button class="button micro warn" data-delete-category="${c.id}">Delete / Disable</button></td></tr>`),'No categories.');
