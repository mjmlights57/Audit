(() => {
  ...
  async function confirmedAction(message,payload,onDone){
  ...
  async function openCustomer(id){
    ...
    $('[data-delete-customer]')?.addEventListener('click',()=>confirmedAction('Delete this customer if unused? If it has business history, EWPros will archive it instead.',{action:'delete_customer',id},async()=>{$('#crmDetailPanel').classList.add('hidden');await loadLookups(true);await loadCustomers();}));
  ...
  async function loadTeam(){
    ...
    $('#workerTable').innerHTML=table(['Worker','Class','Pay type','Rate','Contact','Actions'],state.team.workers.map(w=>`<tr><td><strong>${esc(w.first_name)} ${esc(w.last_name)}</strong></td><td>${esc(w.worker_type)}</td><td>${esc(w.pay_type)}</td><td>${money(w.pay_rate)}</td><td>${esc(w.email||w.phone||'—')}</td><td><button class="button micro warn" data-delete-worker="${w.id}">Delete</button></td></tr>`));
    $$('[data-delete-worker]').forEach(b=>b.addEventListener('click',()=>confirmedAction('Delete this worker if unused? Workers with time, mileage, or payment history will be deactivated instead.',{action:'delete_worker',id:b.dataset.deleteWorker},async()=>{await loadLookups(true);await loadTeam();})));
  ...
  async function readSelectedBankFile(){
    ...
    if(!form||!f)throw new Error('Choose a CSV, OFX, or QFX file.');
  ...
  async function previewSelectedBankFile(){
    ...
    try{
      ...
      if(button){button.disabled=result.summary.newRows===0;button.title=result.summary.newRows===0?'All transactions in this statement are already imported.':'';}
      toast(result.summary.newRows?`${result.summary.newRows} new transaction(s) ready to import`:'No new transactions to import');
      return result;
    }catch(err){state.bankPreview=null;if(button)button.disabled=true;throw err;}
  }
  $('#bankStatementFile')?.addEventListener('change',()=>{
    state.bankFile=null;state.bankPreview=null;$('#bankImportPreview').innerHTML='';
    const hasFile=Boolean($('#bankStatementFile')?.files?.[0]);
    const button=$('#confirmBankImport'); if(button){button.disabled=!hasFile;button.title=hasFile?'Click to import. Preview statement is optional.':'Choose a statement file first.';}
  });
  $('#bankImportAccount')?.addEventListener('change',()=>{state.bankFile=null;state.bankPreview=null;$('#bankImportPreview').innerHTML='';});
  $('#bankImportForm')?.addEventListener('submit',async e=>{e.preventDefault();try{await previewSelectedBankFile();}catch(err){alert(err.message);}});
  $('#confirmBankImport')?.addEventListener('click',async()=>{
    ...
    }catch(err){alert(err.message);}finally{button.textContent='Import transactions';button.disabled=!$('#bankStatementFile')?.files?.[0];}
  });
