...
function renderAppointments() {
  ...
  $('#appointmentsTable').innerHTML = rows.length ? `
    <table><thead><tr><th>Date / Time</th><th>Customer</th><th>Contact</th><th>Address</th><th>Assignment</th><th>Source</th><th>Status</th></tr></thead>
    <tbody>${rows.map(item => `<tr>
      <td>${formatDate(item.date)}${item.scheduledTime?`<br><small>${escapeHtml(item.scheduledTime)}${item.endTime?` – ${escapeHtml(item.endTime)}`:''}</small>`:''}</td>
      <td><strong>${escapeHtml(item.customer)}</strong><small>${escapeHtml(item.appointmentType||'Appointment')} · ${escapeHtml(item.appointmentNumber||item.taskId)}</small></td>
      ...
      <td>${escapeHtml(item.sourceSystem==='crm'?'CRM':'Legacy import')}</td>
      <td>${statusTag(item.crmStatus||item.status, item.active)}</td>
    </tr>`).join('')}</tbody></table>`
...
window.EWPROS_ADMIN_REFRESH = () => loadDashboard(false);
...
function showView(name) {
  ...
  const titles = {
    ...
    team:'Team & Timesheets', mileage:'Mileage', reports:'Reports', workspace:'Audit Workspace', import:'Legacy Asana Import', appointments:'Appointments', history:'Import History'
  };
