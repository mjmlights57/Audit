...
exports.handler = async event => {
  ...
  try {
    ...
    const data = await fetchAll(
      supabase,
      'appointments',
      'id,source_system,external_task_id,appointment_number,customer_id,project_id,business_line_id,assigned_worker_id,appointment_type,appointment_notes,customer_name,customer_phone,customer_email,service_address,scheduled_start,scheduled_end,timezone,appointment_status,source_active,auditor_visible,crm_created,source_payload,updated_at',
      query => query
        .eq('source_active', true)
        .eq('auditor_visible', true)
    ...
    const appointments = data.map(row => {
      ...
      return {
        ...
        companyName: payload.company_name || payload.facility_name || row.customer_name,
        meterId: payload.meter_id || '',
        addressCont: payload.address_cont || '',
        businessType: payload.business_type || '',
        businessSector: payload.business_sector || '',
        buildingType: payload.building_type || payload.business_type || '',
        squareFootage: payload.square_footage || '',
        heatingFuelType: payload.heating_fuel_type || '',
        date: formatDate(row.scheduled_start, sourceDate),
        time: payload.scheduled_time || '',
        auditor: 'auditor',
        sourceSystem: row.source_system || '',
        appointmentType: row.appointment_type || payload.appointment_type || '',
        projectDatabaseId: row.project_id || '',
        customerDatabaseId: row.customer_id || '',
        assignedWorkerId: row.assigned_worker_id || payload.assigned_worker_id || '',
        auditorName: payload.assigned_worker_name || payload.asana_assignee_name || '',
        auditorEmail: payload.assigned_worker_email || payload.asana_assignee_email || '',
