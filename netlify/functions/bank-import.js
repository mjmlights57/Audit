...
exports.handler = async event => {
  ...
  try {
    ...
    const { data:done,error:doneError }=await supabase.from('bank_import_batches').update({status:'completed',imported_rows:imported,duplicate_rows:prepared.rows.length-imported,completed_at:new Date().toISOString(),undone_at:null,undone_rows:0}).eq('id',batch.id).select('*').single();
