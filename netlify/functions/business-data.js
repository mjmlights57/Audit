...
async function dashboard(supabase, lookups) {
  ...
  const [transactions, invoices, allBankTransactions, reminders, profitability] = await Promise.all([
    fetchAll(supabase, 'financial_transactions', 'id,transaction_date,transaction_type,amount,description,business_line_id,project_id,customer_id,financial_account_id,personal', q => q.order('transaction_date', { ascending: false })),
...
async function customersView(supabase, lookups, customerId) {
  ...
  const [projects, invoices, transactions, notes, reminders] = await Promise.all([
    ...
    fetchAll(supabase, 'financial_transactions', '*', q => q.eq('customer_id', customerId).order('transaction_date', { ascending: false })),
...
async function bankingView(supabase, lookups) {
  const [transactions, rules, batches] = await Promise.all([
  ...
  return { accounts: lookups.accounts, categories: lookups.categories, businessLines: lookups.businessLines, customers: lookups.customers, projects: lookups.projects, transactions, rules, batches };
}

async function accountingView(supabase, lookups) {
  const [transactions, invoices, payments] = await Promise.all([
    fetchAll(supabase, 'financial_transactions', '*', q => q.order('transaction_date', { ascending: false }).limit(400)),
  ...
  return { ...lookups, transactions, invoices, payments };
}
...
async function reportsView(supabase, lookups, params) {
  ...
  const transactions = await fetchAll(supabase, 'financial_transactions', 'transaction_date,transaction_type,amount,category_id,business_line_id,customer_id,project_id,vendor_id,financial_account_id,personal', q => q.gte('transaction_date', start).lte('transaction_date', end));
  ...
  const operating = sum(cashTx.filter(t => ['income','customer_payment'].includes(t.transaction_type)), t=>t.amount)
    - sum(cashTx.filter(t => ['expense','vendor_payment'].includes(t.transaction_type)), t=>t.amount);
  const financing = sum(cashTx.filter(t => t.transaction_type==='owner_contribution'), t=>t.amount)
    - sum(cashTx.filter(t => ['owner_draw','credit_card_payment'].includes(t.transaction_type)), t=>t.amount);
