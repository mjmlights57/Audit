...
const sample = {
  ...
  equipment: {
    interior: [
      ...
      { kind: 'lighting', category: 'Lighting', location: 'Sales floor', over300sf: 'Yes', deviceCategory: 'Compact Fluorescents', deviceCode: '1c0005', quantity: 12, photo: null, proposedDevice: '(2x4)LED40W', proposedQty: 12, ctrlQty: 3, ctrlType: 'O+D', notes: 'Replace during phase 1' }
    ],
    exterior: []
  },
...
test('builds a readable CSV with HVAC and lighting rows', () => {
  ...
  assert.match(csv, /Equipment Photo File,Propose Measure,Proposed Quantity,Control Quantity,Control Type/);
  ...
  assert.match(csv, /,3,O\+D,/);
  assert.match(csv, /O\+D/);
});
